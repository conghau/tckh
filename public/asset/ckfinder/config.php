...
//    if ( $config->cfgname == 'app.my_session_name' ) $session_name = $config->cfgvalue;
//}
/*
* access session data from laravel
*/
if ( session_id() == '' ) {
	# use old session
	session_save_path(Config::get('session.files'));
	session_name('htttnb_session');
	session_start();
}

/*
* get user info from larevel
*/
//$config['laravel_userinfo'] = unserialize(Session::get('userinfo'));
$config['laravel_userinfo'] = (json_decode($_SESSION['userinfo']));
//throw new Exception(print_r($_SESSION,true));
//print_r($config);die();
//throw new Exception(print_r(json_decode($_SESSION['userinfo']),true));
$config['is_logined'] = $_SESSION['is_logined'];
...
function CheckAuthentication()
{
	...
	if ( isset($config['is_logined']) && $config['is_logined'] == true ) return true;
	else return false;
}
...
/*
* check if use in module
*/
$config['module_dir'] = '';
$config['module_info'] = null;
if ( isset($_SESSION['ckfinder_module_info']) && $_SESSION['ckfinder_module_info'] != null ) {
	if ( $_SESSION['ckfinder_module_info']['name'] != '' ) $config['module_info'] = $_SESSION['ckfinder_module_info'];	
}
if ( $config['module_info'] != null ) $config['module_dir'] = $config['module_info']['name'] . '/';

if ( $config['module_info'] != null && $config['module_info']['usedb'] == true ) {	
	include_once "plugins/dbfilestorage/plugin.php";
}

$config['userdir'] = 'public';
//if ( $config['laravel_userinfo'] ) $config['userdir'] = $config['laravel_userinfo']->id;
if ( $config['laravel_userinfo'] ) $config['userdir'] = $config['laravel_userinfo']->id;

$baseUrl = str_replace('\\', '/', Config::get('app.url')) . '/asset/ckfinder/userfiles/'.$config['module_dir'].$config['userdir'].'/';
...
*/
#$baseDir = resolveUrl($baseUrl);
$baseDir = str_replace('\\', '/', public_path()) . '/asset/ckfinder/userfiles/'.$config['module_dir'].$config['userdir'].'/';
