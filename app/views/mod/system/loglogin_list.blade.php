...
<script type="text/javascript" src="{{ Config::get('app.url') . '/' . ('asset/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js') }}"></script>
	...
	<div class="headline">
	    <h3 class="clr-o">Hệ thống / Xem Logs đăng nhập</h3>
	</div>
    ...
    <div class="form-group">
        ...
        <div>&nbsp;</div>
        <a class="btn btn-primary" href="javascript:;" onclick="do_search_loginlogs()">Xem logs &raquo;</a>
        <script type="text/javascript">
            ...
            do_search_loginlogs = function() {
                self.location.href = '{{ url("system/requestloginlog") }}?kw='+$('#txtKW').val()+'&tungay='+$('#txtTuNgay').val()+'&denngay='+$('#txtDenNgay').val();
            };
	...
	<table class="table table-bordered responsive-table">
	  <thead>
		<tr>
		  <th>STT</th>
		  <th><span class="caret"><sup>(2)</sup></span>&nbsp;&nbsp;&nbsp;Request username</th>
		  <th>Client info</th>
		  <th>Http referer</th>
		  <th>Http request</th>
		  <th>Result</th>
	  ...
	  <tbody>		
		...
		<tr>
            <td>{{ $stt++ }}</td>
            <td>{{ $log->login_username }}</td>
            <td>{{ $log->client_info }}</td>
            <td>{{ $referer }}</td>
            <td>{{ $log->http_request }}</td>
            <td>{{ $request }}</td>
    ...
    do_delete_all_logs = function() {
        bootbox.confirm('Bạn có chắc là muốn xóa tất cả Logs đăng nhập không?', function(result) {
            if ( result ) {
                self.location.href = "{{ url('system/deleteallloginlog') }}";
            }
        });
    };
	</script>
