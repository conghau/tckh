...
<script type="text/javascript" src="{{ Config::get('app.url') . '/' . ('asset/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js') }}"></script>
	...
	<div class="headline">
	    <h3 class="clr-o">Hệ thống / Xem Logs hoạt động</h3>
	</div>
    ...
    <div class="form-group">
        ...
        <div>&nbsp;</div>
        <a class="btn btn-primary" href="javascript:;" onclick="do_search_activatelogs()">Xem logs &raquo;</a>
        <script type="text/javascript">
            ...
            do_search_activatelogs = function() {
                self.location.href = '{{ url("system/activatelog") }}?kw='+$('#txtKW').val()+'&tungay='+$('#txtTuNgay').val()+'&denngay='+$('#txtDenNgay').val();
            };
            $('#txtKW').keypress(function( event ) {
                if ( event.which == 13 ) {
                    do_search_activatelogs();
    ...
    do_delete_all_logs = function() {
        bootbox.confirm('Bạn có chắc là muốn xóa tất cả Logs hoạt động sau khi đăng nhập  không ?', function(result) {
            if ( result ) {
                self.location.href = "{{ url('system/deleteallactivatelog') }}";
            }
        });
    };
	</script>
